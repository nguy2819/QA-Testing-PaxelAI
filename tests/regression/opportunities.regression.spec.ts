...
test(`Opportunities — ${ROLES_TO_RUN.join('+') || 'all roles'}`, async ({ page }) => {
  initRun();
    ...
    for (const role of ROLES_TO_RUN) {
      ...
      for (const user of roleUsers) {
        ...
        try {
          ...
          await S('Step 3 — Opportunities page loads', async () => {
            await logStep(page, 'Navigating to Opportunities page…', 'running');
            await page.getByRole('button', { name: 'Opportunities' })
              .or(page.getByRole('link', { name: 'Opportunities' }))
              .first()
            ...
            await logStep(
              page,
              `Opportunities heading visible: ${headingVisible ? '✓' : 'FAIL'}`,
              headingVisible ? 'pass' : 'fail'
            );
          });

          // TODO: Add full Opportunities page spec steps here
