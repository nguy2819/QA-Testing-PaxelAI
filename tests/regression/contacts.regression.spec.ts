...
test(`Contacts — ${ROLES_TO_RUN.join('+') || 'all roles'}`, async ({ page }) => {
  initRun();
    ...
    for (const role of ROLES_TO_RUN) {
      ...
      for (const user of roleUsers) {
        ...
        try {
          ...
          await S('Step 3 — Contacts page loads', async () => {
            await logStep(page, 'Navigating to Contacts page…', 'running');
            await page.getByRole('button', { name: 'Contacts' })
              .or(page.getByRole('link', { name: 'Contacts' }))
              .first()
            ...
            await logStep(
              page,
              `Contacts heading visible: ${headingVisible ? '✓' : 'FAIL'}`,
              headingVisible ? 'pass' : 'fail'
            );
          });

          // TODO: Add full Contacts spec steps here
